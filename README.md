...
Reading chat, toggles, and stats need **no key**. Only the moderation buttons (`/modqueue/{id}/approve|reject`) do. Click the **⚷ LOCKED** chip in the top bar and paste your `VEIL_SECRET` once — it's stored in that browser's `localStorage` and sent in the `Authorization` header.

**Don't put the secret in the URL** (`?secret=…`). It would land in the address bar (visible if you ever show your screen), in browser history, and in the server's access logs. For backward-compat, if a `?secret=` is present the cockpit moves it into `localStorage` and strips it from the URL on load — but the initial request is still logged, so prefer the unlock chip.
